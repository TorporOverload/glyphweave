...
def _bootstrap_node_id_index(session_factory: sessionmaker) -> None:
    with session_scope(session_factory) as session:
        ...
        index_names = {
            str(index["name"]) for index in inspect(bind).get_indexes("file_reference")
        }
        if "ix_file_reference_node_id" in index_names:
            return

        session.execute(
            text(
                "CREATE UNIQUE INDEX ix_file_reference_node_id "
                "ON file_reference (node_id) WHERE node_id IS NOT NULL"
            )
        )
