...
def check_and_recover(orchestrator) -> None:
    ...
    try:
        pending_by_file = orchestrator.wal_service.get_unflushed_for_recovery()
        ...
        for file_ref_id, entries in pending_by_file.items():
            try:
                ...
                all_entry_ids.extend(entry.id for entry in entries)
            except Exception as e:
                rt.logger.error(
        ...
        if all_entry_ids:
            orchestrator.wal_service.mark_flushed(all_entry_ids)
            for file_ref_id in pending_by_file:
                orchestrator.wal_service.checkpoint(file_ref_id)
    except Exception as e:
        rt.logger.error(f"Recovery: unexpected error: {e}", exc_info=True)


def replay_entries_for_file(
    orchestrator,
    file_ref_id: int,
    ...
    blob_ids = sorted(
        [blob.blob_id for blob in file_entry.blobs],
        key=lambda blob_id: next(
            blob.blob_index for blob in file_entry.blobs if blob.blob_id == blob_id
        ),
    )

    chunk_store = ChunkStore(
        vault_path=orchestrator.vault_path,
        cache_dir=orchestrator.cache_dir,
        key_service=orchestrator.key_service,
        vault_id=orchestrator.vault_id,
        file_service=orchestrator.file_service,
        folder_service=orchestrator.folder_service,
        gc=orchestrator.gc,
    )
    ...
    for chunk_idx, entry in latest_per_chunk.items():
        data = orchestrator.wal_service.read_chunk_from_wal(entry)
        if data is not None:
