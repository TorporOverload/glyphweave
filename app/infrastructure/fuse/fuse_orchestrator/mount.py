...
def ensure_windows_supported(rt) -> None:
...
def master_key_hex(orchestrator) -> str:
    """Return the master key as a hex string, 
    accepting SecureMemory, bytes, or bytearray."""
    if isinstance(orchestrator.master_key, SecureMemory):
        return orchestrator.master_key.view().hex()
    if isinstance(orchestrator.master_key, bytes):
        return orchestrator.master_key.hex()
    if isinstance(orchestrator.master_key, bytearray):
        return orchestrator.master_key.hex()
    raise TypeError("master_key must be bytes or SecureMemory")


def open_in_default_app(orchestrator, file_path) -> None:
    """Launch the mounted file in the OS default application using shell commands."""
    rt = get_runtime_module()
    ensure_windows_supported(rt)
    try:
        commands = [
...
def mount_and_open(
    ...
    """Start a FUSE mount subprocess for the given file reference
    and optionally open it."""
    rt = get_runtime_module()
    ensure_windows_supported(rt)
    file_ref = orchestrator.file_service.get_file_reference_with_blobs(file_ref_id)
    if not file_ref:
        return None

    with orchestrator._lock:
        if file_ref_id in orchestrator._mounts:
            return orchestrator._mounts[file_ref_id]

    mount_dir = orchestrator.mount_base / f"ref_{file_ref_id}"
    if mount_dir.exists():
        try:
            mount_dir.rmdir()
        except OSError:
    ...
    mounted_file_name = probes.mount_file_name(
        file_ref.name,
        file_ref_id,
        mount_dir,
    ...
    mount_runner = "app.infrastructure.fuse.mount_runner"
    env = os.environ.copy()
    ...
    try:
        ...
        key_payload = bytearray(
            rt.json.dumps(
                {
                    "master_key_hex": master_key_hex(orchestrator),
                    "db_key_hex": orchestrator.key_service.derive_database_key(),
                }
            ).encode("utf-8")
        )
        ...
        cmd = [
            ...
            mount_runner,
            "--vault-id",
            orchestrator.vault_id.decode("utf-8"),
            "--vault-path",
            str(orchestrator.vault_path),
            "--file-ref-id",
            str(file_ref_id),
            "--mount-dir",
            str(mount_dir),
            ...
            mounted_file_name,
            "--vaults-data-dir",
            str(orchestrator.cache_dir.parent.parent),
            "--key-pipe-handle",
    ...
    with orchestrator._lock:
        orchestrator._mounts[file_ref_id] = info

    if open_in_app:
        if not probes.wait_for_mount_path(file_path, timeout=10.0):
            if fuse_process is not None and fuse_process.poll() is None:
                rt.logger.warning(f"Mount path not visible for ref={file_ref_id}")
            orchestrator.unmount(file_ref_id)
            return None

        if not probes.wait_for_mount_office_ready(mount_dir, file_path, timeout=20.0):
            rt.logger.warning(
        ...
        open_in_default_app(orchestrator, file_path)
