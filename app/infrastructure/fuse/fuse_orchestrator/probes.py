...
from app.common.file_extensions import extension_from_mime

from .runtime import get_runtime_module


def wait_for_mount_path(file_path: Path, timeout: float) -> bool:
...
def wait_for_mount_ready(file_path: Path, timeout: float) -> None:
...
def wait_for_mount_responsive(file_path: Path, timeout: float) -> bool:
    """Return True when the mount responds to a probe open within timeout seconds."""
    deadline = time.time() + timeout
    while time.time() < deadline:
        if probe_file_open(file_path, timeout=1.0):
            return True
        time.sleep(0.2)
    return False


def wait_for_mount_office_ready(
    mount_dir: Path,
    file_path: Path,
    timeout: float,
) -> bool:
    ...
    while time.time() < deadline:
        ...
        if not probe_listdir(mount_dir, timeout=1.0):
        ...
        if not probe_file_open(file_path, timeout=1.0):
            get_runtime_module().logger.debug(
        ...
        if not probe_temp_file_ops(mount_dir, timeout=2.0):
            get_runtime_module().logger.debug(
...
def probe_file_open(file_path: Path, timeout: float) -> bool:
...
def probe_listdir(path: Path, timeout: float) -> bool:
    ...
    thread = threading.Thread(target=_try_listdir, daemon=True)
    thread.start()
    thread.join(timeout=timeout)
    return result["ok"]


def probe_temp_file_ops(path: Path, timeout: float) -> bool:
    """Return True if create, rename, and delete operations succeed within timeout."""
    result = {"ok": False}
    ...
    thread = threading.Thread(target=_try_ops, daemon=True)
    thread.start()
    thread.join(timeout=timeout)
    return result["ok"]
...
def mount_file_name(
    ...
    """Derive a safe, length-bounded mount filename from the 
