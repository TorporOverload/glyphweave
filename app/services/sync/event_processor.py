...
class EventProcessor:
    ...
    def process_event(self, event: VaultEvent) -> ProcessingResult:
        ...
        with session_scope(self._session_factory) as session:
            ...
            self._record_processed_event(session, event, event_hash, result)
            return result
    ...
    def _get_ref_by_node_id(self, session, node_id):
        return state.get_ref_by_node_id(session, node_id)

    def _get_parent_ref(self, session, parent_node_id):
        return state.get_parent_ref(self, session, parent_node_id)
    ...
    def _structural_hlc_for_node(self, session, node_id):
        return state.structural_hlc_for_node(session, node_id)

    def _content_hlc_for_node(self, session, node_id):
        return state.content_hlc_for_node(session, node_id)

    def _is_stale_event(self, incoming_hlc, current_hlc):
        return state.is_stale_event(incoming_hlc, current_hlc)

    def _is_deleted_after_or_equal(self, session, node_id, incoming_hlc):
        return state.is_deleted_after_or_equal(session, node_id, incoming_hlc)

    def _upsert_sync_state(self, session, node_id, hlc, structural, content, event_id):
        state.upsert_sync_state(session, node_id, hlc, structural, content, event_id)

    def _record_tombstone(self, session, node_id, node_kind, event):
        state.record_tombstone(session, node_id, node_kind, event)

    def _get_or_create_conflict_folder(self, session):
        return folder_handlers.get_or_create_conflict_folder(session)

    def _conflict_name(self, name, hlc):
