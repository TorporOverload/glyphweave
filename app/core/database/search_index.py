...
from sqlalchemy import DDL, Column, String, Table, event, insert, Result
...
def insert_document_content(session: Session, 
                file_entry_id: str, content: str) -> Result:
    """Insert document content into the search index"""
    search_row = insert(search_index).values(
        file_entry_id=file_entry_id, content=content
    )
    status = session.execute(search_row)
    session.commit()
    return status
