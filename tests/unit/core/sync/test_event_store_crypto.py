...
class TestConcurrentAppendOperations:
    @pytest.mark.skipif(
        __import__("sys").platform == "win32",
        reason="File locking issues on Windows with concurrent writes",
    )
    def test_concurrent_append_events_same_device(self, tmp_path) -> None:
        store = _store(tmp_path / "vault")
        errors: list[Exception] = []

        def append_event(i: int) -> None:
            try:
                event = VaultEvent(
                    event_id=f"evt-concurrent-{i}",
                    type=EventType.FILE_ADD,
                    device_id="device-a",
                    hlc=HybridLogicalClock(
                        wall_time=1000 + i, logical=i, device_id="device-a"
                    ),
                    payload={"index": i},
                    parents=[],
                )
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(append_event, i) for i in range(10)]
            for f in futures:
                f.result()
    ...
    def test_concurrent_append_events_different_devices(self, tmp_path) -> None:
        store = _store(tmp_path / "vault")
        errors: list[Exception] = []
        ...
        def append_event(i: int) -> None:
            try:
                ...
                event = VaultEvent(
                    ...
                    device_id=device_id,
                    hlc=HybridLogicalClock(
                        wall_time=1000 + i, logical=0, device_id=device_id
                    ),
                    payload={"index": i},
                    parents=[],
                )
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            futures = [executor.submit(append_event, i) for i in range(12)]
            for f in futures:
                f.result()
        ...
        for i in range(3):
            ...
            frontier = store.read_frontier(device_id)
    ...
    def test_concurrent_append_and_read(self, tmp_path) -> None:
        store = _store(tmp_path / "vault")
        event = _event("evt-initial")
        stored = store.append_event(event)
        errors: list[Exception] = []
        ...
        def append_event(i: int) -> None:
            try:
                event = VaultEvent(
                    event_id=f"evt-read-{i}",
                    type=EventType.FILE_ADD,
                    device_id="device-a",
                    hlc=HybridLogicalClock(
                        wall_time=2000 + i, logical=i, device_id="device-a"
                    ),
                    payload={"index": i},
                    parents=[stored.event_hash or ""],
                )
                store.append_event(event)
            except Exception as e:
                errors.append(e)
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            append_futures = [executor.submit(append_event, i) for i in range(6)]
            read_futures = [executor.submit(read_event) for _ in range(6)]
            for f in append_futures + read_futures:
                f.result()
